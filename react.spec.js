...
describe("React module", () => {
  ...
  test("allows setting compute cells", () => {
    ...
    // create a cb function that takes `inputCells` (iterable object), access the first element, access the `value` property, then add 1 to the value
    const fn = (inputCells) => inputCells[0].value + 1;
    // create a new ComputeCell object that takes two parameters--inputCell in an array, and a cb function
    const computeCell = new ComputeCell([inputCell], fn);
    // expect computeCell `value` property to equal 2
